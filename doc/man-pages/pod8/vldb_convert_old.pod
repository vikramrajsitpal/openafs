...
vldb_convert_old - Convert the VLDB to/from Transarc AFS versions 3.1-3.4a
...
B<vldb_convert_old> [B<initcmd>] S<<< [B<-to>] <I<AFS version goal>> >>>
...
The B<vldb_convert_old> command is used to convert legacy Transarc 3.1-3.4
...
Run the B<vldb_convert_old> command on the VLDB file using the following
command:

   # vldb_convert_old -path /usr/afs/db/vldb.DB0
