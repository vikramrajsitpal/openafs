...
The server F<NetInfo> file, if present in the F</usr/afs/local> directory (or
the B<-config> directory, if given), defines the following:
...
that appear in the F<NetRestrict> file, if it exists. The File Server records
the resulting list in the F</usr/afs/local/sysid> file and registers the
