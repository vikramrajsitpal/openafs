...
=item B<-ptserver> <I<server>>

Specifies the Protection Server to contact when interacting with the Protection
Database. By default, B<pts> randomly chooses a ptserver in the appropriate
cell, but specifying this option allows you to force using a specific server.
This is normally not needed, but may be useful for testing purposes or if
specific servers are behaving differently.
